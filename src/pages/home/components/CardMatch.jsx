...
import { TrashFill, ChatDotsFill } from 'react-bootstrap-icons'
  ...
  return (
        <>
         <section className="my-3" >
              ...
              <Link to={`../matches/${match?._id}`} className='btn btn-dark btn-sm  mx-1' style={{ fontSize: '12px' }}> Detalles</Link>
              </h1>
               <p style={{ fontSize: '13px' }}>Date: {match?.date?.split('T').reverse().join(' ')} <span><strong>{match?.season?.season}</strong></span></p>
                  <Button className='btn btn-dark btn-sm  d-flex align-items-center' style={{ fontSize: '12px' }} onClick={() => setShow(!show)}>{show ? <>Cerrar comentarios</> : <><ChatDotsFill className='mx-1'/>  {match?.comments?.length}  comentarios</>}</Button>
              </Card.Header>
              <div style={ show ? { maxHeight: '200px', overflow: 'auto', background: 'white', borderRadius: '5px' } : { display: 'none' } }>
                  {match?.comments?.length > 0
                      ...
                      (comment?.username === username)
                        ? <div className="d-flex flex-row justify-content-end m-1 " key={comment?._id}>
                            <div className="p-2 ms-5 border rounded text-end " style={{ margin: '1px', fontSize: '12px' }} >
                                ...
                                {comment?.comment} <TrashFill color='dark' onClick={() => handleRemove(comment?.comment, comment?._id, match?._id)} />
                            </div></div>
                    ...
                    : <Alert variant='info mx-3 m-1 '>No hay comentarios para mostrar!</Alert>}
                 </div>
              <Card.Footer>
                ...
                <div className="float-end mt-1"><Button type='submit' style={{ fontSize: '12px' }} className='btn btn-primarbtn-sm m-1'>Comentar</Button></div>
                </Form>
                </div>
              </Card.Footer>
            </Card>
