...
const SectionPlayers = () => {
  ...
  return (
        ...
        <FormControl style={{ fontSize: '13px' }} name='filter' placeholder='Nombre, deporte, equipo...' onChange={e => setFilter(e.target.value)}/>
        </div>
        {filterPlayers.length > 0
          ? <div className='table-responsive rounded ' style={{ maxHeight: '500px', overflow: 'auto' }}>
             <Table responsive variant='dark table-sm table-borderless' style={{ fontSize: '14px' }} hover>
                ...
                <tbody>
                    {filterPlayers?.map(player => (
                        <tr key={player?._id} onClick={e => navigate(`../players/${player?._id}`)} >
                            <td>
                               {player?.fullName}
                            </td>
                            <td>
                            ...
                            <td>
                                {player?.team ? player?.team?.name : <span className='text-danger'>Sin asignar</span>}
                            </td>
                        </tr>
                    ))}
                </tbody>
